...
- (BOOL)application:(UIApplication *)application willFinishLaunchingWithOptions:(NSDictionary<UIApplicationLaunchOptionsKey,id> *)launchOptions {
    ...
    // Phase 0.2: tell the Swift-side Kernel singleton that boot is starting.
    [[KernelObjCBridge sharedInstance] setPendingBoot];

    bootError = [self boot];

    // Phase 0.2: record the outcome so Swift can observe Kernel.state.
    if (bootError == 0) {
        [[KernelObjCBridge sharedInstance] recordBootSuccess];
    } else {
        NSError *err = [NSError errorWithDomain:@"ish.boot" code:bootError userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"iSH boot failed with error %d", bootError]}];
        [[KernelObjCBridge sharedInstance] recordBootFailure:err];
    }
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    // Phase 0.2: verify KernelObjCBridge state after boot completes
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        NSLog(@"[Velum Phase 0.2] KernelObjCBridge.currentState = %ld (0=unbooted,1=booting,2=ready,3=failed)",
              (long)[KernelObjCBridge sharedInstance].currentState);
    });
